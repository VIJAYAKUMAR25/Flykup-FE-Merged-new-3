...
const handleFailedStatusClick = (video) => {
  ...
  try {
    ...
    setVideos(videosWithUrls); // This will trigger the main useEffect for comprehensive subscription management

    // Optional: Immediate subscription attempt if socket is already connected
    // This can be redundant if the useEffect [videos] is quick, but doesn't harm.
    if (socketRef.current && socketRef.current.connected) {
      videosWithUrls.forEach(video => {
        if (['uploaded', 'processing', 'failed'].includes(video.processingStatus)) {
          console.log(`[Socket.IO Client] (fetchVideos) Attempting to subscribe to video ${video._id} (status: ${video.processingStatus}).`);
          socketRef.current.emit('subscribeToShoppableVideo', { videoId: video._id });
        } else if (video.processingStatus === 'published') {
           console.log(`[Socket.IO Client] (fetchVideos) Video ${video._id} is 'published'. Ensuring unsubscription.`);
          socketRef.current.emit('unsubscribeFromShoppableVideo', { videoId: video._id });
        }
      });
    }
  } catch (error) {
  ...
  //           console.log(`[Socket.IO Client] Subscribing to updates for video ${video._id} (status: ${video.processingStatus})`);
  //           socketRef.current.emit('subscribeToShoppableVideo', { videoId: video._id }, (ack) => {
  //             if (ack && ack.success) {
  //               console.log(`[Socket.IO Client] Successfully subscribed to video ${video._id}: ${ack.message}`);
  //             } else if (ack) {
  //               console.error(`[Socket.IO Client] Failed to subscribe to video ${video._id}: ${ack.message}`);
  //             }
  //           });
  //         }
  //       });
 ...
 useEffect(() => {
  ...
  const handleConnect = () => {
    ...
    videos.forEach(video => {
      if (['uploaded', 'processing', 'failed'].includes(video.processingStatus)) {
        console.log(`[Socket.IO Client] Attempting to subscribe to video ${video._id} (status: ${video.processingStatus}) on connect.`);
        currentSocket.emit('subscribeToShoppableVideo', { videoId: video._id }, (ack) => {
          if (ack && ack.success) {
            console.log(`[Socket.IO Client] Successfully subscribed to video ${video._id} on connect: ${ack.message}`);
          } else if (ack) {
            console.error(`[Socket.IO Client] Failed to subscribe to video ${video._id} on connect: ${ack.message}`);
          }
        });
      } else if (video.processingStatus === 'published') {
        // Ensure unsubscription if already published on connect (e.g., after quick reconnect)
        console.log(`[Socket.IO Client] Video ${video._id} is 'published' on connect. Ensuring unsubscription.`);
        currentSocket.emit('unsubscribeFromShoppableVideo', { videoId: video._id });
      }
    });
  };
  ...
  const handleStatusUpdate = (update) => {
    console.log("[Socket.IO Client] Received shoppableVideoStatusUpdate:", update);
    if (update && update.videoId && update.statusDetails) {
      ...
      if (newStatus === 'published') {
        console.log(`[Socket.IO Client] Video ${update.videoId} has become 'published'. Unsubscribing.`);
        currentSocket.emit('unsubscribeFromShoppableVideo', { videoId: update.videoId });
        ...
        console.log(`[Socket.IO Client] Video ${update.videoId} status is now '${newStatus}'. Ensuring subscription.`);
        currentSocket.emit('subscribeToShoppableVideo', { videoId: update.videoId });
      }
    }
  };
  ...
  if (currentSocket.connected) {
    videos.forEach(video => {
      if (['uploaded', 'processing', 'failed'].includes(video.processingStatus)) {
        console.log(`[Socket.IO Client] (videos effect) Ensuring subscription for video ${video._id} (status: ${video.processingStatus}).`);
        currentSocket.emit('subscribeToShoppableVideo', { videoId: video._id });
      } else if (video.processingStatus === 'published') {
        // If a video is marked as published in the current `videos` state, ensure we are not subscribed.
        console.log(`[Socket.IO Client] (videos effect) Video ${video._id} is 'published'. Ensuring unsubscription.`);
        currentSocket.emit('unsubscribeFromShoppableVideo', { videoId: video._id });
      }
    });
  }
  ...
  return () => {
    console.log("[Socket.IO Client] Cleaning up socket connections and listeners for ViewShopable component...");
    ...
    videos.forEach(video => {
      ...
      if (['uploaded', 'processing', 'failed', 'published'].includes(video.processingStatus)) {
        console.log(`[Socket.IO Client] Unsubscribing from video ${video._id} on unmount (original status: ${video.processingStatus}).`);
        currentSocket.emit('unsubscribeFromShoppableVideo', { videoId: video._id });
      }
    });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Assuming thumbnailURL and videoToDelete.originalVideoBlobName are keys for S3 deletion
      // The schema has 'originalVideoBlobName', not 'videoURL' directly for the raw video file.
      // Adjust if your 'videoToDelete' object structure differs or 'videoURL' refers to something else.
      const s3DeletePromises = [];
      if (videoToDelete.thumbnailBlobName) { // Prefer blob name if available
        s3DeletePromises.push(deleteObjectFromS3(videoToDelete.thumbnailBlobName));
      } else if (videoToDelete.thumbnailURL && videoToDelete.thumbnailURL.includes('amazonaws.com')) { // Fallback if it's a direct S3 URL
      ...
      // If there's an HLS manifest, deleting individual segments might be complex and usually handled by backend logic or TTL policies.
      // For now, focusing on thumbnail and original video.
      ...
      if (socketRef.current && socketRef.current.connected) {
        socketRef.current.emit('unsubscribeFromShoppableVideo', { videoId: videoToDelete._id });
      }
    } catch (error) {
  ...
  const handleVisibilityChange = async (videoId, currentVisibility) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        // Update with server response data if it contains more than just visibility
        setVideos((prevVideos) =>
          prevVideos.map((video) =>
 ...
 const VideoStatus = ({ status, videoData, onClick }) => { // Added videoData and onClick props
    switch (status) {
      case "queued":
        return (
      ...
      case "uploaded": // Assuming 'uploaded' is similar to 'published'
        return (
          <span className="flex items-center gap-1.5 text-sm text-green-600 font-medium">
            <CheckCircle2 className="w-4 h-4" />
            Published.
  ...
  return (
    <>
      ...
      <div className="flex flex-col min-h-screen w-full bg-blackLight shadow-2xl relative overflow-hidden p-6 text-gray-800">
        {/* Header Section */}
        <div className="sticky top-0 z-10 bg-blackLight p-4 shadow-sm flex justify-between items-center mb-4 -mx-6 px-6">
          <h1 className="text-2xl font-bold text-whiteLight">
            Shoppable Videos
          </h1>
          ...
          <table className="table w-full">
            <thead className="bg-newYellow text-blackDark text-md font-bold">
              <tr>
                ...
                <th className="p-4 text-center font-bold">Active/Deactive</th>
                ...
                videos.map((video) => {
                  ...
                  return (
                    <tr
                      ...
                      <td className="p-4 font-medium truncate max-w-xs">
                        {video.title}
                      </td>
                      ...
                      <td className="p-4 whitespace-nowrap text-sm text-whiteLight">
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            // disabled={video.processingStatus !== 'published' && video.processingStatus !== 'failed'} // Allow edit if published or failed
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-md">
          <motion.div
            ...
            className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg text-gray-800" // Increased max-w-lg for more content
          >
            ...
            <div className="mb-6">
              ...
              <pre className="text-sm text-red-700 bg-red-50 p-3 rounded whitespace-pre-wrap break-all max-h-60 overflow-y-auto border border-red-200">
                {videoForFailureModal.processingError || "No specific error message provided."}
              </pre>
            </div>
            <div className="flex justify-end gap-3">
              ...
              {/* <button
                onClick={() => {
                  ...
                  console.log("Retry processing for video:", videoForFailureModal._id);
                  setFailureModal(false);
                  setVideoForFailureModal(null);
                }}
                className="btn btn-warning bg-primaryYellow hover:bg-yellow-300 text-newBlack px-4 py-2 rounded-md transition-colors"
              >
                Retry Processing
              </button> */}
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-gray-700 mb-6">
                Are you sure you want to delete the video "<strong>{videoToDelete.title}</strong>"? This action cannot be undone.
              </p>
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => setDeleteModal(false)}
