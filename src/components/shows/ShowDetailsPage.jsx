...
const ShowDetailsPage = ({ requireAuth, isAuthenticated, currentUser }) => {
  ...
  const fetchShowInitial = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        ].filter((p) => getProductIdSafely(p.productId)); 

        if (
          showData.currentGiveaway &&
          getProductIdSafely(showData.currentGiveaway.productId)
        ) {
          allProductsToSign.push({
            productId: showData.currentGiveaway.productId,
          });
        }
  ...
  useEffect(() => {
    ...
    const handleGiveawayStarted = (data) => {
      ...
      setShow((prevShow) => {
        ...
        const newGiveawayProductId = getProductIdSafely(data.productId);
        ...
        const updatedGiveawayProducts = prevShow.giveawayProducts.map((gp) => {
          ...
          if (
            historicalProductId &&
            newGiveawayProductId &&
            historicalProductId === newGiveawayProductId
          ) {
            return {
              ...gp,
              isActive: true,
    ...
    const handleGiveawayApplicantsUpdated = (data) => {
      setShow((prevShow) => {
        ...
        const currentGiveawayProductId = getProductIdSafely(
          prevShow.currentGiveaway.productId
        );
        if (
          incomingProductId &&
          currentGiveawayProductId &&
          incomingProductId === currentGiveawayProductId
        ) {
          return {
            ...prevShow,
            currentGiveaway: {
    ...
    const handleGiveawayWinner = ({
      streamId: winStreamId,
      ...
      if (winStreamId === id) {
        ...
        setShow((prevShow) => {
          ...
          const updatedHistoricalGiveawayProducts =
            prevShow.giveawayProducts.map((gp) => {
              const historicalProductId = getProductIdSafely(gp.productId);

              if (
                historicalProductId &&
                incomingWinProductId &&
                historicalProductId === incomingWinProductId
              ) {
                return {
                  ...gp,
                  isActive: false,
                  ...
                  winner: newWinner, 
                  applicants:
                    prevShow.currentGiveaway?.applicants || gp.applicants, 
                };
              }
              return gp;
            });
          ...
          return {
            ...
            giveawayProducts: updatedHistoricalGiveawayProducts, 
          };
        });
    ...
    const handleGiveawayEndedManually = ({
      streamId: endStreamId,
      productId: endProductId,
      productTitle,
      message,
    }) => {
      if (endStreamId === id) {
        ...
        setShow((prevShow) => {
          ...
          const incomingEndProductId = getProductIdSafely(endProductId);
          ...
          const updatedHistoricalGiveawayProducts =
            prevShow.giveawayProducts.map((gp) => {
              ...
              if (
                historicalProductId &&
                incomingEndProductId &&
                historicalProductId === incomingEndProductId
              ) {
                return {
                  ...gp,
                  isActive: false,
                  ...
                  applicants:
                    prevShow.currentGiveaway?.applicants || gp.applicants, // Preserve applicants for historical record
                };
              }
              return gp;
            });
    ...
    return () => {
      if (socket) {
        ...
        socket.off(
          ...
          handleGiveawayApplicantsUpdated
        );
  ...
  const currentAuctionProduct = show?.auctionProducts?.find(
    (p) =>
      ...
      getProductIdSafely(currentAuction?.product)
  );
      ...
      <div className="w-[25%] hidden lg:block border-r border-stone-800 bg-stone-950 text-white shadow-xl">
            ...
            <div className="avatar">
              {show?.host?.userInfo?.profileURL ? (
                ...
                >
                  <img
                    src={
                      `${import.meta.env.VITE_AWS_CDN_URL}${
        ...
        <div className="p-6 space-y-6">
          ...
          >
            ...
            {activeTab === "Giveaway" && (
              <div className="space-y-4">
                {show?.currentGiveaway &&
                show.currentGiveaway.isActive &&
                ...
                {show?.giveawayProducts?.filter((p) => p.isGiveawayEnded)
                  .length > 0 && (
                  <div className="mt-8 pt-4 border-t border-stone-800">
                    ...
                    {show.giveawayProducts
                      ...
                      .map((completedProduct) => (
                        <div
                          key={getProductIdSafely(completedProduct.productId)}
                          className="flex items-center gap-3 p-3 bg-stone-900 rounded-lg mb-2"
                        >
                          <img
                            src={
                              signedUrls[
                                getProductIdSafely(completedProduct.productId)
                              ] || "/placeholder.svg"
                            }
                          ...
                          <div className="flex-1">
                            <p className="font-semibold text-sm">
                              {completedProduct.productTitle}
                            </p>
                            {completedProduct.winner ? (
                              <p className="text-xs text-green-400 flex items-center gap-1">
                                <Trophy size={12} /> Winner:{" "}
                                {completedProduct.winner.userName ||
                                  completedProduct.winner.name ||
                                  "Unknown"}
                              </p>
                            ) : (
                    ...
                    {show?.giveawayProducts?.filter((p) => p.isGiveawayEnded)
                      ...
                      <div className="mt-8 pt-4 border-t border-stone-800">
                        ...
                        {show.giveawayProducts
                          ...
                          .map((completedProduct) => (
                            <div
                              key={getProductIdSafely(
                                completedProduct.productId
                              )}
                              className="flex items-center gap-3 p-3 bg-stone-900 rounded-lg mb-2"
                            >
                              <img
                ...
                {show?.host?.userInfo?.profileURL?.key ? (
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        const firstLetter =
                          show?.host?.userInfo?.userName?.charAt(0) || "U";
                    ...
                    {show?.giveawayProducts?.length ? (
                      show?.giveawayProducts.map((taggedProduct) => (
                        ...
                        >
                          ...
                          {show.currentGiveaway &&
                          getProductIdSafely(show.currentGiveaway.productId) ===
                            getProductIdSafely(taggedProduct.productId) &&
                          show.currentGiveaway.isActive &&
                          !show.currentGiveaway.isGiveawayEnded ? (
                            <GiveAwayUsers
                              streamId={id}
                              product={show.currentGiveaway}
                              signedUrls={signedUrls}
                              socket={socket}
                            />
                          ) : (
