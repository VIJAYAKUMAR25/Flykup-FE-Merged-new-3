...
const ForgotPassword = ({ onViewChange, isModal = false }) => {
  const navigate = useNavigate();
  const [step, setStep] = useState(1);
  const [email, setEmail] = useState('');
  const [otp, setOtp] = useState('');
  ...
  return (
    <div className={`${isModal ? 'w-full' : 'min-h-full flex items-center justify-center'}`}>
      <div className={`w-full max-w-md space-y-1 ${isModal ? 'bg-transparent' : 'bg-blackLight'} ${!isModal ? 'rounded-xl shadow-xl' : ''} ${isModal ? 'p-0' : 'p-2'}`}>
        <div className="flex justify-between items-center">
          <button 
            onClick={handleBack} 
            className="group flex items-center gap-2 px-2 text-gray-600 hover:text-gray-800 transition-colors"
          >
            <IoArrowBackCircleOutline className={`h-8 w-8 ${isModal ? 'text-gray-400 group-hover:text-white' : 'text-newYellow group-hover:text-whiteLight'} transition-colors`} /> 
          </button>
        </div>

        <div className="text-center">
          <div className="text-center pb-3">
            <h1 className={`text-2xl font-thin ${isModal ? 'text-white' : 'text-whiteLight'} tracking-wide mb-2 relative`}>
              RESET<span className="font-black text-newYellow"> PASSWORD</span>
            </h1>
            <div className="w-24 h-px bg-gradient-to-r from-transparent via-newYellow to-transparent mx-auto"></div>
          </div>
          <p className={`${isModal ? 'text-gray-300' : 'text-whiteHalf'}`}>Step <span className='text-greenLight'>{step}</span> of 4</p>
        </div>

        {error && (
          <div className={`${isModal ? 'bg-red-900/50 border-red-500' : 'bg-red-50 border-red-400'} border-l-4 px-4 py-2 rounded-md`}>
            <p className={`${isModal ? 'text-red-300' : 'text-red-700'}`}>{error}</p>
          </div>
        )}
          ...
          <form onSubmit={handleSendOtp} className="space-y-6">
            <div className="relative">
              <input
                ...
                onChange={(e) => setEmail(e.target.value)}
                placeholder=" "
                className={`peer w-full px-4 py-3 border-2 rounded-lg ${isModal ? 'bg-gray-700 text-white border-gray-600' : 'bg-blackDark text-whiteLight border-gray-500'}
                  backdrop-blur-sm placeholder-transparent font-bold
                  focus:border-amber-300 focus:outline-none 
                  focus:ring-2 focus:ring-blue-400/20 transition-all duration-200`}
              />
              <label
                className={`absolute left-2 -top-2.5 px-2 text-sm font-medium ${isModal ? 'bg-gray-800' : 'bg-whiteLight'} rounded-xl
                    ${isModal ? 'text-gray-300' : 'text-gray-600'} transition-all duration-200
                    peer-placeholder-shown:text-base ${isModal ? 'peer-placeholder-shown:text-gray-400' : 'peer-placeholder-shown:text-blackDark'}
                    peer-placeholder-shown:top-3 peer-placeholder-shown:left-4
                    peer-focus:-top-2.5 peer-focus:left-2 peer-focus:text-sm
                    ${isModal ? 'peer-focus:text-gray-300' : 'peer-focus:text-blackDark'}`}
              >
                Email Address
              </label>
            </div>
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOtp} className="space-y-3">
            <div className="flex flex-col items-center justify-center p-2 space-y-2">
              ...
              <div className="text-center space-y-1">
                <h1 className={`text-xl font-bold ${isModal ? 'text-white' : 'text-whiteLight'}`}>Check your inbox</h1>
                <p className={`${isModal ? 'text-gray-300' : 'text-gray-200'} max-w-sm text-sm`}>
                  We've sent a 6-digit OTP to your email address. 
                  Please check your inbox and enter the code below.
                </p>
              </div>
              ...
              <div className="flex space-x-3">
                {[0, 150, 300, 450, 600, 750].map((delay, index) => (
                  <span key={index} className="w-2 h-2 bg-newYellow rounded-full animate-bounce" style={{ animationDelay: `${delay}ms` }}></span>
                ))}
              </div>
              ...
              <div className="relative w-full">
                <input
                  ...
                  placeholder=" "
                  maxLength={6}
                  className={`peer w-full px-4 py-2 border-2 ${isModal ? 'text-white bg-gray-700 border-gray-600' : 'text-whiteLight bg-blackDark border-gray-200'} rounded-lg
                    backdrop-blur-sm placeholder-transparent text-center tracking-wider
                    focus:border-amber-300 focus:outline-none 
                    focus:ring-2 focus:ring-blue-300/20 transition-all duration-200`}
                />
                <label
                  className={`absolute left-2 -top-2.5 px-2 text-sm font-medium ${isModal ? 'bg-gray-800 text-gray-300' : 'bg-whiteLight text-gray-600'} rounded-xl
              ...
              <div className="text-center">
                <p className={`text-sm ${isModal ? 'text-gray-300' : 'text-gray-200'}`}>
                  Didn't receive the OTP?{' '}
                  <button
                    type="button"
                    onClick={handleResendOtp}
                    disabled={!canResend || isSendingOtp}
                    className={`font-medium ${
                      canResend ? 'text-green-400 hover:text-green-300' : 'text-gray-300 cursor-not-allowed'
                    }`}
                  >
                    {isSendingOtp ? 'Sending...' : 'Resend OTP'}
                    {!canResend && ` (${countdown}s)`}
          ...
          <form onSubmit={handleResetPassword} className="space-y-6 pt-4">
            <div className="relative">
              <input
                ...
                onChange={(e) => setPassword(e.target.value)}
                placeholder=" "
                className={`peer w-full px-4 py-3 border-2 ${isModal ? 'text-white bg-gray-700 border-gray-600' : 'text-whiteLight bg-blackDark border-gray-600'} rounded-lg
                  backdrop-blur-sm placeholder-transparent
                  focus:border-amber-300 focus:outline-none 
                  focus:ring-2 focus:ring-amber-300/20 transition-all duration-200`}
              />
            ...
            <div className="w-full max-w-md">
              <div className="relative">
                <div className="relative text-center space-y-2">
                  ...
                  <div className="flex justify-center space-x-4 py-1">
                    <div className={`flex items-center space-x-2 ${isModal ? 'text-gray-300' : 'text-gray-200'}`}>
                      <LockKeyhole className="w-5 h-5 text-green-500" />
                      <span className="text-sm">Encrypted</span>
                    </div>
                    <div className={`flex items-center space-x-2 ${isModal ? 'text-gray-300' : 'text-gray-200'}`}>
                  ...
                  <p className={`text-sm ${isModal ? 'text-gray-300' : 'text-gray-200'} pt-2`}>
                    For enhanced security, please make sure to remember your new password
                  </p>
                </div>
              </div>
