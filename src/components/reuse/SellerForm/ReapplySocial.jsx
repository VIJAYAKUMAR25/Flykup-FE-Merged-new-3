...
import { GET_USER_DETAILS_BY_ID, UPDATE_SELLER_APPLICATION ,IMAGE_UPLOADTO_AZURE} from "../../api/apiDetails.js";
...
const FirstTabContent = React.memo(
  ({ handleCheckboxChange, formData, handleInputChange, handleBusinessTypeSelect, errors, }) => (
    <div className="lg:p-6 p-3 bg-black/5 rounded-lg shadow-md">
      <h2 className="text-2xl text-newBlack font-bold mb-2">Social Seller Information</h2>
      ...
      <div className="mb-4">
        <label className="flex items-center space-x-3">
          <input
            type="checkbox"
            name="isAgeConfirmed"
            checked={formData.isAgeConfirmed || false}
            onChange={handleCheckboxChange}
            className="h-5 w-5 rounded border-gray-300"
          />
          <span className="text-sm text-newBlack">
    ...
    return (
      <div className="p-6 bg-black/5 rounded-lg shadow-md">
       ...
       <div className="mb-8 bg-white p-4 rounded-lg shadow-md">
        ...
        {/* Add What Do You Want to Sell Section */}
        <div className="mb-8 bg-white p-4 rounded-lg shadow-md">
          <label className="block text-newBlack font-semibold mb-3 flex items-center">
            <Tag size={18} className="mr-2" />
            What Do You Want to Sell? (Select Multiple)
          </label>
          <div className="grid lg:grid-cols-2 gap-3">
            {sellingTypes.map((type) => (
              <button
                key={type}
                onClick={() => {
                  const currentTypes = formData.wantToSell || [];
                  const updatedTypes = currentTypes.includes(type)
                    ? currentTypes.filter(t => t !== type)
                    : [...currentTypes, type];
                  setFormData({ ...formData, wantToSell: updatedTypes });
                }}
                className={`p-3 rounded-lg border flex items-center justify-between ${formData.wantToSell?.includes(type)
                  ? "bg-gray-900 text-white border-gray-900"
                  : "border-gray-300 bg-newWhite text-newBlack hover:border-gray-900"
                  }`}
              >
                <span>{type}</span>
                <span>
                  {formData.wantToSell?.includes(type) ? (
                    <Check size={16} />
                  ) : (
                    <Circle size={16} />
                  )}
        ...
        <div className="mb-8 bg-white p-4 rounded-lg shadow-md">
          <label className="block text-newBlack font-semibold mb-3 flex items-center">
            <Globe size={18} className="mr-2" />
            Would You Like Flykup to Help You Get Brand Collaborations?
          </label>
          <div className="flex gap-4 mb-4">
            <button
              onClick={() => setFormData({ ...formData, wantBrandCollaborations: true })}
              className={`px-8 py-2 rounded ${formData.wantBrandCollaborations === true
            ...
            <button
              onClick={() => setFormData({ ...formData, wantBrandCollaborations: false })}
              className={`px-8 py-2 rounded ${formData.wantBrandCollaborations === false
                ? "bg-gray-900 text-white"
                : "bg-white text-gray-900"
                }`}
            >
          ...
          {errors.wantBrandCollaborations && (
            <span className="text-red-500 text-sm block mt-1">
              {errors.wantBrandCollaborations}
            </span>
          )}
...
const ReapplySocialSeller = () => {
  ...
  const validateTab = useCallback((tab) => {
    ...
    if (tab === 0) {
      ...
      if (!formData.isAgeConfirmed) {
        newErrors.isAgeConfirmed = "You must be 18 or older to proceed";
      }
      ...
      if (formData.wantBrandCollaborations === null) {
        newErrors.wantBrandCollaborations = "Please select an option";
      }
  ...
  const formatSubmissionData = () => {
    return {
      isAdult: Boolean(formData.isAgeConfirmed),
      ...
      wantToSell: formData.wantToSell || [],
      ...
      shippingInfo: {
        ...
        dispatchTime: formData.dispatchTime === "same_day" ? "24 hours" : formData.dispatchTime, // Formatting example
      ...
      promotions: {
        ...
        flykupCollab: formData.wantBrandCollaborations // Mapped to new field
      },
      address: {
...
export default ReapplySocialSeller;
