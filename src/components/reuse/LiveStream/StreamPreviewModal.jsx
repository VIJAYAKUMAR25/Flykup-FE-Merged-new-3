...
const StreamPreviewModal = ({
  ...
  const getDevices = async () => {
    try {
      ...
      setSelectedDevices((prev) => ({
        camera:
          prev.camera || (cameras.length > 0 ? cameras[0].deviceId : null),
        microphone:
          prev.microphone ||
          (microphones.length > 0 ? microphones[0].deviceId : null),
        speaker:
          prev.speaker || (speakers.length > 0 ? speakers[0].deviceId : null),
      }));

      addLog(
        `Found ${cameras.length} cameras, ${microphones.length} microphones, ${speakers.length} speakers`
      );
    } catch (error) {
      ...
      try {
        ...
        addLog(
          `Fallback device detection failed: ${fallbackError.message}`,
          "error"
        );
      }
    }
  ...
  const checkPermissions = async () => {
    try {
      ...
      addLog(
        `Permissions - Camera: ${cameraPermission.state}, Microphone: ${microphonePermission.state}`
      );
    } catch (error) {
  ...
  const requestUserMedia = async () => {
    try {
      ...
      const hasMicrophone =
        selectedDevices.microphone && devices.microphones.length > 0;

      if (!hasCamera && !hasMicrophone) {
        throw new Error(
          "No camera or microphone selected. Please select at least one device."
        );
      }
        ...
        audio: hasMicrophone
          ? {
              deviceId: selectedDevices.microphone
                ? { exact: selectedDevices.microphone }
                : undefined,
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "error":
        return "text-red-500"; // Keep standard red for errors
      case "denied":
        return "text-red-500"; // Keep standard red for denied
      default:
        return "text-whiteHalf"; // Use whiteHalf for pending/not tested
    }
  };
        ...
        <div className="bg-blackLight text-whiteLight rounded-xl w-full max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex justify-between items-center p-6 border-b border-greyLight">
            <div>
              ...
              <p className="text-whiteHalf mt-1">
                Test your camera, microphone, and stream settings before going
                live
              </p>
            </div>
            <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className={`p-2 rounded-full ${
                          isVideoMuted ? "bg-red-600" : "bg-greyLight"
                      ...
                      <button
                        ...
                        className={`p-2 rounded-full ${
                          isAudioMuted ? "bg-red-600" : "bg-greyLight"
                        } hover:bg-opacity-80 transition`}
                ...
                <div className="bg-blackDark p-4 rounded-lg">
                  <h3 className="font-semibold mb-3 text-whiteLight">
                    Quality Settings
                  </h3>
                  <div className="space-y-3">
                    <div>
                      <label className="block text-sm font-medium mb-1 text-whiteLight">
                        Video Quality
                      </label>
                      <select
                        value={videoQuality}
                        ...
                        className={`select select-bordered focus:select-focus w-full bg-blackDark ${
                          videoQuality ? "text-whiteLight" : "text-whiteHalf"
                        } focus:border-newYellow focus:ring-1 focus:ring-newYellow transition-all duration-200`}
                      >
                        {Object.keys(videoQualityPresets).map((quality) => (
                          <option key={quality} value={quality}>
                            {quality} ({videoQualityPresets[quality].width}x
                            {videoQualityPresets[quality].height})
                          </option>
                        ))}
                      </select>
                    </div>
                    <div>
                      ...
                      <select
                        ...
                        className={`select select-bordered focus:select-focus w-full bg-blackDark ${
                          audioQuality ? "text-whiteLight" : "text-whiteHalf"
                        } focus:border-newYellow focus:ring-1 focus:ring-newYellow transition-all duration-200`}
                      >
                        {Object.keys(audioQualityPresets).map((quality) => (
                          <option key={quality} value={quality}>
                            {quality.charAt(0).toUpperCase() + quality.slice(1)}{" "}
                            ({audioQualityPresets[quality].sampleRate}Hz)
                          </option>
                        ))}
                      </select>
                    </div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      {devices.cameras.length === 0 && (
                        <p className="text-whiteHalf text-xs mt-1">
                    ...
                    {!selectedDevices.camera && !selectedDevices.microphone && (
                      <div className="bg-yellowHalf border border-newYellow p-3 rounded-md">
                        <p className="text-whiteLight text-sm">
                          ⚠️ Please select at least one device (camera or
                          microphone) to start the preview.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-blackDark p-4 rounded-lg">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-whiteLight">
                        Camera Test
                      </span>
                      <span
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        ...
                        {testResults.microphone || "Not tested"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-whiteLight">
                      ...
                      >
                        ...
                        {testResults.connection || "Not tested"}
                      </span>
                    </div>
                  </div>
                ...
                {/* Error Display */}
                {/* Calculate if there are any *displayable* errors first */}
                {(() => {
                  const displayableErrors = Object.entries(errors).filter(
                    ([_, error]) => error
                  );
                    ...
                    return (
                      <div className="bg-red-900 border border-red-700 p-4 rounded-lg">
                        ...
                        <div className="space-y-1">
                          {displayableErrors.map(([key, error]) => (
                  ...
                  return null; // Don't render anything if no displayable errors
                })()}
              </div>
            </div>
          </div>
