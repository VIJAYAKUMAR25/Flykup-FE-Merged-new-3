...
const AuctionsUser = ({ showId, streamId, product, signedUrls, fetchShow, currentAuction }) => {
  ...
  return (
 <div className="shadow-lg max-w-xs sm:max-w-sm mx-auto text-white">
  <div className="flex justify-between items-center mb-1.5">
    {isActive && (
      <span className={`text-sm sm:text-base font-bold ${timer / 1000 <= 11 ? 'text-red-500' : 'text-green-400'}`}>
        {formatTime(timer)}
      </span>
    )}
  </div>
  
  <div className="flex items-center gap-2 sm:gap-3 bg-stone-800 p-2 sm:p-3 rounded-lg">
    <img 
      src={signedUrls[product?.productId?._id] || "https://5.imimg.com/data5/SELLER/Default/2024/1/377210111/AL/EY/GR/50801707/0aa-12.jpg"}
      className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 object-cover rounded-md shadow-md flex-shrink-0"
      alt={product?.productId?.title}
    />
    <div className="min-w-0 flex-1">
      <h3 className="text-sm sm:text-base font-semibold truncate">{product?.productId?.title}</h3>
      <p className="text-xs text-gray-300 mt-0.5 line-clamp-2">{product?.productId?.description}</p>
    </div>
  </div>
  ...
  <div className="text-center mt-2 sm:mt-3">
    {isActive && (
      <>
        ...
        <p className="text-lg sm:text-xl font-bold text-yellow-400 flex justify-center items-center gap-1">
    ...
    <AnimatePresence>
      {bidderWon ? (
        <motion.div 
          ...
          className="flex items-center justify-center gap-1 mt-1.5 sm:mt-2 text-yellow-400"
        >
          <Trophy className="w-3.5 h-3.5" />
          <span className="text-xs font-medium truncate">Winner: {bidderWon.name}</span>
        </motion.div>
      ) : highestBidder && (
        <motion.div 
          initial={{ opacity: 0, y: 10 }} 
        ...
        >
          <Trophy className="w-3.5 h-3.5" />
          <span className="text-xs font-medium truncate">Highest: {highestBidder.name}</span>
        </motion.div>
      )}
    </AnimatePresence>
  </div>
...
<div className="hidden flex justify-center gap-2 sm:gap-3 mt-2 sm:mt-3">
    <button 
      onClick={() => handleBid(nextBid1)} 
      disabled={!isActive || !user || user?._id === highestBidder?._id} 
      className="bg-yellow-500 hover:bg-yellow-600 text-black px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition duration-300"
    >
      Bid <IndianRupee size={10} className="inline ml-0.5" /> {nextBid1.toLocaleString()}
    </button>
    <button 
      ...
      className="bg-yellow-500 hover:bg-yellow-600 text-black px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition duration-300"
    >
      Bid <IndianRupee size={10} className="inline ml-0.5" /> {nextBid2.toLocaleString()}
    </button>
  </div>
    </div>
  );
