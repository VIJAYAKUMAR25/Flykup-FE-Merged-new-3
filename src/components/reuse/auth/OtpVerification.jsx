...
const OtpVerification = ({ phone, email, onViewChange, isModal = false }) => {
  ...
  return (
    <div className={`${isModal ? 'w-full' : 'mt-5 p-1'}`}>
      <div className={`w-full max-w-md ${isModal ? 'bg-transparent' : 'bg-white'} rounded-lg ${!isModal ? 'shadow-md' : ''} ${isModal ? 'p-0' : 'p-6'}`}>
        {!isModal && (
          <div className='flex justify-between'>
            <h2 className="text-2xl font-bold text-center mb-6 text-primaryNavy">Verify OTP</h2>
            <Link className="text-black" to={"/"}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="text-center space-y-2">
            <p className={`text-sm ${isModal ? 'text-gray-300' : 'text-gray-600'}`}>
              Enter the verification code we sent to
            </p>
            <p className={`font-medium ${isModal ? 'text-whiteLight' : 'text-gray-800'}`}>{phone || email}</p>
          </div>
          ...
          <div className="flex justify-center space-x-1">
            {otp.map((digit, index) => (
              <input
                ...
                onKeyDown={(e) => handleKeyDown(index, e)}
                onFocus={(e) => handleFocus(index, e)}
                className={`w-12 h-12 text-center border ${isModal ? 'bg-gray-700 text-white border-gray-600' : 'bg-inputYellow text-primaryBlack border-gray-300'} font-bold rounded-md text-lg focus:outline-none focus:ring-2 ${isModal ? 'focus:ring-amber-300' : 'focus:ring-blue-500'}`}
                maxLength={6}
              />
            ))}
          </div>
          ...
          {error && (
            <p className={`text-center text-sm ${isModal ? 'text-red-400' : 'text-red-600'}`}>{error}</p>
          )}

          <button
            type="submit"
            className={`w-full ${isModal ? 'bg-newYellow text-blackDark hover:bg-amber-300' : 'bg-blue-600 text-white hover:bg-blue-700'} py-2 px-4 rounded-md focus:outline-none focus:ring-2 ${isModal ? 'focus:ring-amber-300' : 'focus:ring-blue-500'} focus:ring-offset-2 transition-colors`}
          >
            Verify
          </button>
          ...
          <div className="text-center space-y-2">
            <p className={`text-sm ${isModal ? 'text-gray-300' : 'text-gray-600'}`}>
              ...
              <button
                type="button"
                onClick={handleResendOTP}
                className={`font-bold ${timer === 0 ? (isModal ? 'text-green-400 hover:text-green-300' : 'text-blue-600 hover:text-blue-800') : 'text-gray-400 cursor-not-allowed'}`}
                disabled={timer > 0}
              >
                Resend
              </button>
