...
const EditProfileModal = ({ onProfileUpdate }) => {
  ...
  const handleImageUpload = async (
    ...
    try {
      ...
      console.error(`${imageType} S3 upload failed:`, error);
      setMessage(
        `${imageType} upload failed: ${error.message || "Server error"}`
      );
      setMessageType("error");
      urlSetter(
        imageType === "profile"
          ? user?.profileURL || { key: "" }
          : user?.backgroundCoverURL || { key: "" }
      );
    } finally {
      uploaderSetter(false);
    }
  ...
  const disableSave =
    ...
    isUploading ||
    checkingUsername ||
    (messageType === "error" && userName.trim() !== originalUsername.current);
  ...
  return (
    <>
      ...
      >
        <LuPencil size={16} />
        <span className="ml-2" style={{ color: colors.newYellow }}>
          Edit Profile
        </span>
      </button>
        ...
        {isOpen && (
          ...
          >
            ...
            <motion.div
              ...
              exit={{ scale: 0.9, opacity: 0, y: 20 }}
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
              className="relative z-10 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
              style={{ backgroundColor: colors.blackDark }}
            >
              <header
                className="p-4 border-b flex justify-between items-center flex-shrink-0"
                style={{ borderColor: colors.blackLight }}
              >
                <h2
                  className="text-xl font-bold"
                  style={{ color: colors.newYellow }}
                >
                  Edit Profile
                </h2>
                <button
                  onClick={closeModal}
                  className="p-2 rounded-full text-gray-500 hover:bg-gray-700 transition-colors"
                  disabled={isUploading || loading}
                  style={{ color: colors.whiteHalf }}
                >
                  <CgClose size={20} />
                </button>
              </header>
              ...
              <main className="flex-grow overflow-y-auto grid grid-cols-1 lg:grid-cols-3 gap-x-8">
                ...
                <div className="col-span-1 p-6 space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative w-32 h-32 mx-auto group">
                      <div
                        className="h-full w-full rounded-full ring-4 ring-white bg-gray-600 overflow-hidden"
                        style={{ ringColor: colors.blackLight }}
                      >
                        {profileURL.key ? (
                          <img
                            src={`${CDNURL}${profileURL.key}`}
                            alt="Profile"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-32 bg-gray-700 rounded-lg relative group">
                      {backgroundCoverURL.key && (
                        <img
                          src={`${CDNURL}${backgroundCoverURL.key}`}
                          alt="Background"
                ...
                {/* --- Right Column: Form Fields --- */}
                <div
                  className="col-span-1 lg:col-span-2 p-6 border-t lg:border-t-0 lg:border-l space-y-6"
                  style={{ borderColor: colors.blackLight }}
                >
                  <div>
                    <label
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="input-style pl-10 w-full p-2.5 rounded-lg border focus:outline-none"
                        style={{
                          backgroundColor: colors.blackLight,
                          borderColor: colors.blackLight,
                          color: colors.whiteLight,
                          "--tw-ring-color": colors.newYellow, // Apply focus ring color
                        }}
                    ...
                    <p
                      className={`text-xs mt-1.5 min-h-[1rem] flex items-center ${
                        messageType === "success"
                          ? "text-green-500"
                          : messageType === "error"
                          ? "text-red-500"
                    ...
                    >
                      ...
                      {checkingUsername && (
                        <BiLoaderAlt className="animate-spin inline-block ml-2" />
                      )}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <textarea
                        ...
                        className="input-style h-32 pl-10 resize-none w-full p-2.5 rounded-lg border focus:outline-none"
                        style={{
                          backgroundColor: colors.blackLight,
                          borderColor: colors.blackLight,
                          color: colors.whiteLight,
              ...
              >
                ...
                <div className="flex items-center">
                  <button
                    onClick={closeModal}
                    className="btn btn-ghost bg-yellow-700 border-b mr-2"
                    disabled={isUploading || loading}
                    style={{ color: colors.whiteLight, "--tw-bg-opacity": 0 }}
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="btn text-white min-w-[120px] transition-all duration-300"
                    style={{
                      backgroundColor: colors.newYellow,
                      color: colors.blackDark, // Changed text color for better contrast on yellow
                      border: "none",
                      opacity: disableSave ? 0.6 : 1, // Dim when disabled
